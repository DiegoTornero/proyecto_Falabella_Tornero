...
class Empresa(Base):
    ...
    tipo_empresa        = Column(String(20), default="micro")      # micro | pequena | mediana | grande
...
class Credito(Base):
    ...
    id = Column(String, primary_key=True, default=gen_uuid)
    usuario_id = Column(String, ForeignKey("usuarios.id", ondelete="CASCADE"), index=True, nullable=True)   # nullable: créditos empresariales no tienen usuario
    empresa_id = Column(String, ForeignKey("empresas.id", ondelete="CASCADE"), index=True, nullable=True)   # nullable: créditos personales no tienen empresa
