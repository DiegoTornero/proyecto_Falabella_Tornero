...
const AVATARES = [
  { id: "avatar_default", emoji: "👤", nombre: "Predeterminado" },
  { id: "leon", emoji: "🦁", nombre: "León Falabella" },
  { id: "halcon", emoji: "🦅", nombre: "Halcón Falabella" },
  { id: "buho", emoji: "🦉", nombre: "Búho Falabella" },
  { id: "zorro", emoji: "🦊", nombre: "Zorro Falabella" },
  { id: "panda", emoji: "🐼", nombre: "Panda Falabella" }
]

export default function Perfil() {
  const { user, logout } = useAuth()
  const [usuario, setUsuario] = useState(null)
  const [form, setForm] = useState({ nombre: "", apellido: "", telefono: "", direccion: "", avatar: "avatar_default" })
  ...
  const [passForm, setPassForm] = useState({ password_actual: "", nueva_password: "" })
  ...
  const handleChangePassword = async (e) => {
    e.preventDefault()
    setPassError("")
    if (passForm.nueva_password.length !== 6) {
      setPassError("La nueva clave debe ser de exactamente 6 dígitos numéricos.")
      return
    }
    try {
      await api.put(`/usuarios/${user.id}/password`, passForm)
      setMensaje("✅ Clave de Internet cambiada exitosamente. Redirigiendo al login...")
      setShowPasswordModal(false)
      setPassForm({ password_actual: "", nueva_password: "" })
      setTimeout(() => {
        logout()
      }, 1500)
    } catch (err) {
      setPassError(err.response?.data?.detail || "Error al cambiar contraseña")
    }
  }

  const iniciales = usuario ? `${usuario.nombre?.charAt(0) || ""}${usuario.apellido?.charAt(0) || ""}`.toUpperCase() : "U"
  const avatarSeleccionado = AVATARES.find(a => a.id === usuario?.avatar) || AVATARES[0]
    ...
    <Layout title="Mi Perfil" subtitle="Gestiona tu información personal y configuración de seguridad">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 p-8 text-center relative overflow-hidden">
            ...
            <div className="w-28 h-28 bg-gradient-to-br from-[#0a1f14] to-[#16422b] rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg shadow-[#0a1f14]/20 relative z-10 border-4 border-white text-5xl">
              {avatarSeleccionado.id === "avatar_default" ? (
            ...
            <div className="bg-[#f0f5e6] rounded-2xl px-4 py-3 border border-[#c8e000]/30 inline-flex items-center gap-2 mb-6">
            ...
            <div className="border-t border-gray-100 pt-6">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Elige tu Avatar Premium</p>
              <div className="flex flex-wrap gap-2.5 justify-center">
                {AVATARES.map(av => (
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await api.put(`/usuarios/${user.id}`, { avatar: av.id })
                        setMensaje("✅ Avatar premium actualizado")
                        fetchData()
                      } catch (err) {
                        setMensaje("❌ Error al guardar avatar")
                      }
                    }}
                    title={av.nombre}
                    className={`w-11 h-11 rounded-full flex items-center justify-center text-xl transition-all border-2 ${
                      usuario?.avatar === av.id ? 'border-[#c8e000] bg-gray-100 scale-110 shadow' : 'border-transparent bg-gray-50 hover:bg-gray-100'
                    }`}
                  >
                    {av.emoji}
                  </button>
          ...
          <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 p-8">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => { setShowPasswordModal(true); setPassError(""); }} className="w-full flex items-center gap-4 text-left p-4 rounded-2xl border border-gray-100 hover:border-[#c8e000] hover:bg-gray-50 transition-all group">
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 bg-[#0a1f14]/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-fade-up">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-[#0a1f14]">Cambiar Clave</h3>
              <button onClick={() => setShowPasswordModal(false)} className="text-gray-400 hover:text-gray-600 text-2xl font-semibold">&times;</button>
            </div>
            ...
            <form onSubmit={handleChangePassword} className="flex flex-col gap-5">
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Clave Actual</label>
                <input
                  type="password"
                  maxLength={6}
                  required
                  value={passForm.password_actual}
                  onChange={e => setPassForm({ ...passForm, password_actual: e.target.value.replace(/\D/g, "") })}
                  placeholder="******"
                  className="w-full mt-2 border border-gray-200 rounded-xl px-4 py-3 text-center text-lg font-bold bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#c8e000]"
                />
              </div>

              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Nueva Clave (6 dígitos)</label>
                <input
                  type="password"
                  maxLength={6}
                  required
                  value={passForm.nueva_password}
                  onChange={e => setPassForm({ ...passForm, nueva_password: e.target.value.replace(/\D/g, "") })}
                  placeholder="******"
                  className="w-full mt-2 border border-gray-200 rounded-xl px-4 py-3 text-center text-lg font-bold bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#c8e000]"
                />
              </div>
              ...
              <div className="flex gap-3 mt-2">
                <button type="button" onClick={() => setShowPasswordModal(false)} className="flex-1 py-3 border-2 border-gray-100 text-gray-500 rounded-xl font-bold text-sm hover:bg-gray-50">
                  Cancelar
                </button>
                <button type="submit" className="flex-1 py-3 bg-[#0a1f14] text-[#c8e000] rounded-xl font-bold text-sm hover:bg-black">
                  Guardar Clave
                </button>
              </div>
            </form>
