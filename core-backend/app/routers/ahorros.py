...
class EstadoCuentaSchema(BaseModel):
    estado: str

@router.put("/{cuenta_id}/estado")
def cambiar_estado_cuenta(cuenta_id: int, data: EstadoCuentaSchema, db: Session = Depends(get_db), trabajador=Depends(get_current_trabajador)):
    cuenta = db.query(CuentaAhorro).filter(CuentaAhorro.id == cuenta_id).first()
    if not cuenta:
        raise HTTPException(status_code=404, detail="Cuenta no encontrada")
    
    nuevo_estado = data.estado.upper()
    if nuevo_estado not in ["ACTIVA", "BLOQUEADA"]:
        raise HTTPException(status_code=400, detail="Estado inválido. Debe ser ACTIVA o BLOQUEADA")
        
    cuenta.estado = nuevo_estado
    db.commit()
    db.refresh(cuenta)
    return {"mensaje": f"Estado de cuenta actualizado a {nuevo_estado} exitosamente", "estado": cuenta.estado}
